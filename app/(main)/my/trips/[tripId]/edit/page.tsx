...
export default function TripDetailPage({ params }: TripDetailPageProps) {
    ...
    async function loadTrip() {
      ...
      if (result.success && result.data) {
        ...
        if (result.data.itinerary) {
          ...
          // 첫 번째 일차를 기본 선택
          if (result.data.itinerary.length > 0) {
            setSelectedDayForPlace(result.data.itinerary[0].dayNumber);
          }
        }
      } else {
  ...
  const handleAddPlaceToItinerary = async () => {
    ...
    const addResult = await addPlace({
      ...
      lat: selectedPlace.coordinate.lat,
      lng: selectedPlace.coordinate.lng,
    ...
    // 2. 선택한 일차의 일정 마지막에 장소 추가
    const dayItinerary = editedItineraries.find(
      (it) => it.dayNumber === selectedDayForPlace,
    );

    if (!dayItinerary) {
      showErrorToast("일차를 찾을 수 없습니다.");
      return;
    }

    // 3. 새로운 ScheduleItem 생성 (마지막에 추가)
    const newScheduleItem: ScheduleItem = {
      order: dayItinerary.schedule.length + 1,
      placeId: addResult.data.id,
      placeName: addResult.data.name,
      arrivalTime: "10:00", // 임시, 재계산 시 업데이트됨
      departureTime: "11:00", // 임시, 재계산 시 업데이트됨
      duration: addResult.data.estimatedDuration,
      isFixed: false,
          ...
          <div className="space-y-4">
            ...
            {trip?.itinerary && trip.itinerary.length > 0 && (
              <div className="space-y-2">
                <label className="text-sm font-medium">일차 선택</label>
                <Select
                  value={String(selectedDayForPlace)}
                  onValueChange={(value) =>
                    setSelectedDayForPlace(Number(value))
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
