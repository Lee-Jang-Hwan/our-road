...
**저장 기능**:
- 마이페이지에서 저장된 일정 목록 확인
- 여행 상세 정보 및 최적화된 일정 조회

**공유 기능** (`/share/[tripId]`):

**핵심 원칙**:
1. **비로그인 사용자도 일정 조회 가능**: 로그인 없이 공유 링크를 통해 여행 일정을 볼 수 있음
2. **최적화된 일정만 공유**: 최적화된 일정(itinerary)이 있는 여행만 공유 가능. 일정이 최적화되지 않은 경우 공유 불가

**공유 페이지 기능**:
- 일자별 탭으로 일정 표시 (일정표 생성과 동일한 UI)
- 지도에 경로 표시 (폴리라인, 마커)
- 공유 링크 복사/공유 (Web Share API 또는 클립보드)
- "나도 계획하기" CTA로 회원가입 유도

**보안 및 프라이버시**:
- Service Role을 사용하여 RLS 우회 (읽기 전용)
- 민감한 정보 제거: `userId`, `fixedSchedules` (고정 일정)는 공유하지 않음
- 최적화된 일정만 표시 (초안 상태의 여행은 공유 불가)
...
### 3.5 경로 재계산 vs 경로 최적화

본 서비스에서는 **경로 재계산**과 **경로 최적화**를 명확하게 구분합니다.

#### 3.5.1 경로 재계산 (Route Recalculation)

**목적**: 사용자가 수동으로 장소 순서를 변경한 후, 변경된 순서에 맞게 경로 정보만 업데이트

**특징**:
- **순서 결정**: 사용자가 수동으로 변경 (드래그 앤 드롭)
- **순서 변경**: 사용자가 변경한 순서를 그대로 유지
- **경로 정보**: 변경된 순서에 맞게 경로 정보(거리, 시간, polyline 등)만 탐색/업데이트
- **최적화 알고리즘**: 실행하지 않음

**사용 시점**:
- 편집 모드(`/my/trips/[tripId]/edit`)에서 사용자가 드래그 앤 드롭으로 순서 변경 후
- "경로 재계산" 버튼 클릭 시

**구현 파일**: `actions/itinerary/recalculate-routes.ts`

**동작 과정**:
1. 현재 일정의 순서 유지 (사용자가 변경한 순서)
2. 각 구간별로 경로 정보 재사용 가능 여부 판단
3. 경로 정보가 없는 구간만 API 호출 (Kakao Mobility API 또는 ODsay API)
4. 거리 행렬 업데이트 (차량 모드)
5. `transportToNext` 업데이트
6. 시간 재계산 (`recalculateItineraryTimes`)
7. DB 저장

#### 3.5.2 경로 최적화 (Route Optimization)

**목적**: 최적화 알고리즘을 실행하여 장소 순서를 자동으로 재계산하고 전체 일정을 최적화

**특징**:
- **순서 결정**: 알고리즘이 자동으로 재계산 (Nearest Neighbor + 2-opt)
- **순서 변경**: 알고리즘이 장소 순서를 재배치
- **경로 정보**: 최적화된 순서에 맞게 경로 정보 생성/업데이트
- **최적화 알고리즘**: Nearest Neighbor, 2-opt 등 실행

**사용 시점**:
- `draft` 또는 `optimizing` 상태일 때 자동 실행 (`/my/trips/[tripId]`)
- 장소 추가/삭제 후 `/plan/[tripId]`에서 `/my/trips/[tripId]`로 이동 시 자동 실행
- 사용자가 수동으로 "최적화" 버튼 클릭 시

**구현 파일**: `actions/optimize/optimize-route.ts`

**동작 과정**:
1. 거리 행렬 계산 (`createDistanceMatrix`)
2. Nearest Neighbor로 초기 경로 생성
3. 2-opt로 경로 개선 (최대 100회 반복)
4. 고정 일정 제약 조건 반영
5. 일자별 분배 (일일 시간 제한 고려)
6. 각 구간 이동 정보 조회 (API 호출)
7. DB 저장
...
| 구분 | 경로 재계산 | 경로 최적화 |
|------|------------|------------|
| **순서 결정** | 사용자 수동 | 알고리즘 자동 |
| **순서 변경** | 사용자가 변경한 순서 유지 | 알고리즘이 순서 재계산 |
| **경로 정보** | 업데이트만 수행 | 생성/업데이트 모두 수행 |
| **최적화 알고리즘** | 실행하지 않음 | Nearest Neighbor + 2-opt 실행 |
| **사용 위치** | `/my/trips/[tripId]/edit` | `/my/trips/[tripId]`, `/plan/[tripId]` |
| **트리거** | 사용자가 "경로 재계산" 버튼 클릭 | 상태 기반 자동 실행 또는 사용자 요청 |

**핵심 원칙**:
- **경로 재계산**: 사용자가 설정한 순서를 존중하며, 그 순서에 맞는 정확한 경로 정보만 제공
- **경로 최적화**: 장소 목록을 기반으로 효율적인 방문 순서를 자동으로 계산하여 전체 일정을 최적화
